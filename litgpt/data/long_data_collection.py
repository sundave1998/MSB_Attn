...
@dataclass
class LongDataCollection(DataModule):
    """The long data collections data module for pretraining."""

    data_path: Union[str, Path] = Path("/media/ybsun/Storage/Long-Data-Collections-main/processed")
    ...
    def tokenize(self, data: Dataset, index: int):
    ...
    def prepare_data(self) -> None:
        ...
        # data_dir = "/media/ybsun/Storage/Long-Data-Collections-main/processed"
        dataset = load_dataset("arrow", data_files=os.path.join(self.data_path, "*.arrow"), num_proc=(os.cpu_count() // 2), trust_remote_code=True)
        # dataset = load_dataset("arrow", data_files=os.path.join(self.data_path, "*.arrow"))        
        ...
        optimize(
            fn=partial(self.tokenize, split_dataset["train"]),
            inputs=list(range(len(split_dataset["train"]))),
            output_dir=self.data_path_train,
            # num_workers=min(0, os.cpu_count() - 1),
            num_workers=min(64, os.cpu_count() - 1),
            chunk_bytes="200MB",
        )
        optimize(
            fn=partial(self.tokenize, split_dataset["val"]),
